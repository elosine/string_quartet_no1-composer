...
const socketWithFallback = [
    ...
    '        const sock = {',
    '            id: "local-offline",',
    '            _loopStartMs: null, _loopEndMs: null, _loopEnabled: false, _loopCount: 0,',
    '            on(event, fn) {',
    ...
    '                        isPlaying: true,',
    '                        currentScoreTimeMs: _scoreTimeMs,',
    '                        scoreTimeOffset: goNow - _scoreTimeMs,',
    '                        tempoHistory: _tempoHistory,',
    '                        serverTime: goNow,',
    '                        scheduledStartTime: goNow + 10',
    '                    });',
    ...
    '                        serverTime: Date.now()',
    '                    });',
    '                } else if (event === "loopSet") {',
    '                    if (data.point === "A") { sock._loopStartMs = data.timeMs; }',
    '                    else if (data.point === "B") {',
    '                        sock._loopEndMs = data.timeMs;',
    '                        if (sock._loopStartMs !== null && sock._loopEndMs < sock._loopStartMs) {',
    '                            var t = sock._loopStartMs; sock._loopStartMs = sock._loopEndMs; sock._loopEndMs = t;',
    '                        }',
    '                    }',
    '                    sock._loopCount = 0;',
    '                    sock._trigger("loopState", { loopStartMs: sock._loopStartMs, loopEndMs: sock._loopEndMs, loopEnabled: sock._loopEnabled, loopCount: 0 });',
    '                } else if (event === "loopToggle") {',
    '                    if (sock._loopStartMs === null || sock._loopEndMs === null) return;',
    '                    sock._loopEnabled = !sock._loopEnabled;',
    '                    if (sock._loopEnabled) sock._loopCount = 0;',
    '                    sock._trigger("loopState", { loopStartMs: sock._loopStartMs, loopEndMs: sock._loopEndMs, loopEnabled: sock._loopEnabled, loopCount: sock._loopCount });',
    '                } else if (event === "loopClear") {',
    '                    sock._loopStartMs = null; sock._loopEndMs = null; sock._loopEnabled = false; sock._loopCount = 0;',
    '                    sock._trigger("loopState", { loopStartMs: null, loopEndMs: null, loopEnabled: false, loopCount: 0 });',
    '                } else if (event === "setLeader" || event === "recallAll") {',
    ...
    '            }',
    '        };',
    '        // Stub loop check (mirrors server-side 200ms interval)',
    '        setInterval(function() {',
    '            if (!_isPlaying || !sock._loopEnabled) return;',
    '            if (sock._loopStartMs === null || sock._loopEndMs === null) return;',
    '            var now = Date.now();',
    '            var scoreMs = _playStartScoreMs + (now - _playStartRealMs);',
    '            if (scoreMs >= sock._loopEndMs) {',
    '                _scoreTimeMs = sock._loopStartMs;',
    '                _isPlaying = false;',
    '                sock._loopCount++;',
    '                sock._trigger("scoreGoto", { isPlaying: false, currentScoreTimeMs: sock._loopStartMs, targetSeconds: sock._loopStartMs / 1000, tempoHistory: _tempoHistory, serverTime: now });',
    '                setTimeout(function() {',
    '                    _isPlaying = true;',
    '                    _playStartRealMs = Date.now();',
    '                    _playStartScoreMs = sock._loopStartMs;',
    '                    var loopGoNow = Date.now(); sock._trigger("scoreGo", { isPlaying: true, scoreTimeOffset: loopGoNow - sock._loopStartMs, currentScoreTimeMs: sock._loopStartMs, serverTime: loopGoNow, scheduledStartTime: loopGoNow + 10 });',
    '                    sock._trigger("loopState", { loopStartMs: sock._loopStartMs, loopEndMs: sock._loopEndMs, loopEnabled: true, loopCount: sock._loopCount });',
    '                }, 10);',
    '            }',
    '        }, 200);',
    '        // Offline mode: auto-assign as leader after a short delay',
    '        setTimeout(function() { sock._trigger("leaderChange", { leaderId: "local-offline" }); }, 200);',
    '        return sock;',
    '    };',
    '}',
    '</script>'
...
const calcSyncReplacement = [
    ...
    '                }',
    '                ',
    '                // Phase 13: NTP-style best-quartile selection + weighted averaging',
    '                // When ≥8 samples: sort by RTT, take best 25% (lowest RTTs = most symmetric)',
    '                // When <8: use all samples (Phase 6 fallback)',
    '                var useIndices = [];',
    '                if (this.syncSamples.length >= 8) {',
    '                    var indexed = [];',
    '                    var rttArr = this._rttSamples;',
    '                    for (var ii = 0; ii < rttArr.length; ii++) indexed.push(ii);',
    '                    indexed.sort(function(a, b) { return rttArr[a] - rttArr[b]; });',
    '                    var quartileCount = Math.max(2, Math.ceil(this.syncSamples.length / 4));',
    '                    useIndices = indexed.slice(0, quartileCount);',
    '                } else {',
    '                    for (var ii = 0; ii < this.syncSamples.length; ii++) useIndices.push(ii);',
    '                }',
    '                var weightedSum = 0, totalWeight = 0;',
    '                for (var qi = 0; qi < useIndices.length; qi++) {',
    '                    var idx = useIndices[qi];',
    '                    var w = 1 / (1 + this._rttSamples[idx]);',
    '                    weightedSum += this.syncSamples[idx] * w;',
    '                    totalWeight += w;',
    '                }',
...
const pongHandlerWithDriftCheck = [
    ...
    '                });',
    '                ',
    "                // Phase 13: Heartbeat listener — fast disconnect detection",
    "                this.socket.on('heartbeat', (data) => {",
    '                    this._lastHeartbeat = performance.now();',
    '                    this._heartbeatMissed = false;',
    '                });',
    '                // Phase 13: Heartbeat watchdog — check every 1s for 3s gap',
    '                if (!this._heartbeatWatchdog) {',
    '                    var self = this;',
    '                    this._heartbeatWatchdog = setInterval(function() {',
    '                        if (!self.connected || !self._lastHeartbeat) return;',
    '                        var elapsed = performance.now() - self._lastHeartbeat;',
    '                        if (elapsed > 3000 && !self._heartbeatMissed) {',
    '                            self._heartbeatMissed = true;',
    '                            console.warn("ClockSync: heartbeat lost (" + Math.round(elapsed) + "ms) — needs re-sync");',
    '                            self._updateSyncUI();',
    '                        }',
    '                    }, 1000);',
    '                }',
...
const clockSyncGetRTTReplacement = [
    ...
    '            ',
    '            // Phase 13: Heartbeat tracking',
    '            _lastHeartbeat: 0,',
    '            _heartbeatMissed: false,',
    ...
    '                    // Phase 13: Override to orange if heartbeat lost',
    '                    if (this._heartbeatMissed) { color = "#f80"; label = "heartbeat lost"; }',
    '                    this._syncQuality = label;',
    '                    el.style.background = color;',
    '                    var hbAge = this._lastHeartbeat ? Math.round(performance.now() - this._lastHeartbeat) : 0;',
    '                    el.title = "Sync: " + label +',
    ...
    '                        " | heartbeat: " + (hbAge > 0 ? hbAge + "ms ago" : "n/a");',
    '                }',
    '            }'
].join('\n');
...
const perfCss = [
    ...
    '        /* Phase 8 Stage 5c: Hide entire right panel (controls overlay replaces it) */',
    ...
    '        #ScoreContainer svg * { cursor: default !important; }',
