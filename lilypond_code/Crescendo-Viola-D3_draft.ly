...
\paper{
  tagline = ##f
  paper-width = 14.5\mm %adjust paper-width,line-width @bottom, and \time 4/4 to get proper display width
  paper-height = 20\mm
  top-margin = 0\mm
  bottom-margin = 0\mm
  left-margin = 1\mm
...
\score {
  ...
  }
  {
    ...
    %Hairpin Adjustments
    \override Hairpin.minimum-length = #0.0
    \once \override DynamicText.extra-spacing-width = #'(+inf.0 . -inf.0)
       % Make hairpin taller/wider
    \override Hairpin.height = #1.1  % Increase height (default ~0.66)
    ...
    -\tweak shorten-pair #'(0 . 1.5)    % (left . right) - positive=shorter
    \<
    -\tweak extra-offset #'(0 . -0.8)  % X,Y offset in staff spaces
    ^\markup {
    ...
    ds4
    s2\!% D-sharp to end the glissando and hairpin
    % NOTATION HERE /////////////////////////////////////////////////////
