  ...
  \score {
    ...
    <<
...

      {
        ...
        % --- PRESSURE WEDGE DEFINITION ---
        % This creates the tapering graphic for bow pressure
        \define  pressureWedge = {
          \once \override TextSpanner.stencil = 
            #(lambda (grob)
               (let* ((width (interval-length (ly:grob-extent grob grob X)))
                      (start-thick 1.4)   % Thickness at start (max pressure)
                      (end-thick 0.1)     % Thickness at end (normal)
                      (stencil (ly:make-stencil
                                `(polygon 
                                   (list 0 ,(/ start-thick 2)
                                         ,width ,(/ end-thick 2)
                                         ,width ,(/ end-thick -2)
                                         0 ,(/ start-thick -2))
                                   #t) 
          ...
          \once \override TextSpanner.padding = #3 % Height above staff
        }
        ...
        % 1. Call the wedge, then start the note and the spanners
        \pressureWedge
        a''2\ppp\<\startTextSpan \glissando 
        ^\markup { 
          \override #'(font-name . "Crimson Pro Light Italic") 
          \fontsize #-6 
          "Non-Vib / max. pressure"
        } 

        % 2. Hidden target note for 1/4 tone drop
        \once \override NoteHead.transparent = ##t
        \once \override Accidental.transparent = ##t
        \once \override NoteHead.no-ledgers = ##t
        
        eeh'''4\! \stopTextSpan ^\markup {
          \override #'(font-name . "Crimson Pro Light Italic")
          \fontsize #-6
          \center-column {
            "secco (ord.)"
        ...
        % 3. Spacer to end the hairpin/timing
        s4\!
...
%         \once \override NoteHead.no-ledgers = ##t
%         
%         % Use Unicode arrow (↓) for maximum compatibility
%         eeh'''4\! ^\markup {
%           \override #'(font-name . "Crimson Pro Light Italic")
%           \fontsize #-6
