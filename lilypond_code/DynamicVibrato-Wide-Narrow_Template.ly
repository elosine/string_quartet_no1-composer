...
\score {
  ...
  }
  {
    ...
    \override Hairpin.Y-offset = #-0.9  % Move hairpin y
    ...
    % Vibrato: wide (3.0) -> non-vibrato (0.0)
    % Adjust amplitudes, wavelength, thickness, and width-fraction to taste:
    %   #'(3.0 0.0) = wide -> none    |  #'(0.5 3.0 0.0) = narrow -> wide -> none
    %   #1.0 = wavelength             |  #0.15 = line thickness
    %   #0.85 = use 85% of available width (adjust to fit hairpin)
    ...
    %Dymanic
    -\tweak extra-offset #'(0 . 0.3)  % X,Y offset in staff spaces
    \fff
    
    % Hairpin
    -\tweak extra-offset #'(-0.5 . 1.2)  % X,Y offset in staff spaces
    ...
    % Non-Vib Text (positioned at right end of vibrato)
    -\tweak extra-offset #'(-5.5 . 3.5)  % X,Y offset in staff spaces (adjust X to align with vibrato end)
    ^\markup {
     ...
     %Dymanic
    -\tweak extra-offset #'(-8 . 0.3)  % X,Y offset in staff spaces
    \f
