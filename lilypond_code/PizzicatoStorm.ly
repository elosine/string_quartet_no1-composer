...
#(define instrument-name-font-size -3)
...
% Stem Y-extent: (bottom . top) in staff spaces relative to staff center
% Adjust these to control where the stem starts and ends independently
% Negative = below staff center, Positive = above staff center
#(define pluck-stem-bottom 0)   % bottom of stem
#(define pluck-stem-top 13)     % top of stem (increase to reach top notehead)

% Instrument name padding (X distance from staff, in staff spaces)
% Decrease this value to move names closer to staff
#(define instrument-name-padding -1)
  ...
  \score {
    <<
      ...
      }
      {
        ...
        % Y-extent controls stem endpoints independently: (bottom . top)
        \override Stem.Y-extent = #`(,pluck-stem-bottom . ,pluck-stem-top)
